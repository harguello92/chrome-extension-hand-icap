const cautionHandlerBuilder = ({ button }) => {
  const hideButton = (visible) => (button.hidden = visible);
  const setScale = (scale) => (button.style.scale = scale);
  const setOpacity = (opacity) => (button.style.opacity = opacity);
